...
/* 定义一个workqueue结构体 */
DECLARE_WORK(user_key_wq, work_queue_func);
...
irqreturn_t user_key_isr(int irq, void *dev_id)
{
	...
	schedule_work(&user_key_wq);
	return IRQ_WAKE_THREAD;
}
irqreturn_t user_key_thread_irq(int irq, void *dev_id)
{
	INF("threaded irq user key botoom half\n");
	return IRQ_HANDLED;
}
/* 中断下半部处理函数，tasklet实现 */
void tasklet_func(unsigned long data)
{
	INF("tasklet user key bottom half\n");
}

void work_queue_func(struct work_struct *work)
{
	INF("work queue user key bottom half!\n");
}
...
int user_key_probe(struct platform_device *pdev)
{
	...
	for (idx = 0; idx < count; idx++) {
		...
		#else
		ret = request_threaded_irq(user_key_desp[idx].irq_num, user_key_isr, user_key_thread_irq, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "user_keys", (void *)&user_key_desp[idx]);
		#endif
		if (ret) {
