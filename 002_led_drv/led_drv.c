...
#define DEVICE_NAME    "alpha_led"
#define CLASS_NAME     "alpha_led_class"
...
static volatile unsigned int *IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO03 = NULL;
...
static ssize_t led_drv_write(struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
    ...
    status ? (*GPIO1_DR |= (0x1 << 3)) : (*GPIO1_DR &= ~(0x1 << 3));
...
static int led_drv_open(struct inode *node, struct file *file)
{
    if (!CCM_CCGR1)
    {
        ...
        IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO03 = ioremap(0x20E02F4, 4);
    ...
    *IOMUXC_SW_MUX_CTL_PAD_GPIO1_IO03 |=  (0x5 << 0);
    ...
    /* 配置GPIO1_IO3功能 */
    *IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO03 &= ~(0xFFFF << 0);
    *IOMUXC_SW_PAD_CTL_PAD_GPIO1_IO03 |=  (0x10B0 << 0);
...
static int __init led_drv_init(void)
{
    ...
    device_create(led_class, NULL, MKDEV(major, 0), NULL, DEVICE_NAME"0");
